...
# @router(r"/home/?(.*)\.html\??(.*)")
@router(r"/home/?(\w*)?\.html\??(\w*)?")
def home(rematch):
    # try:
    page = int(rematch.group(1)) if rematch.group(1)!="" else 0
    word = rematch.group(2)
    # except Exception:
    #     page = rematch.group(1)
    ...
    data_cur.execute(f"SELECT * FROM de_en81683 limit {page}, 20")
    total_dic = data_cur.fetchall()#return a tuple('','','')
    # data_cur.execute(
    ...
    if word:
        print(f"**********************{word=}")
        data_cur.execute(f"SELECT * FROM de_en81683 where German = '{word}';")
        print(f"SELECT * FROM de_en81683 where German = '{word}';")
        search_res = data_cur.fetchall()
        print(f"**********************{search_res=}")
    data_cur.close()
    data_conn.close()
    ...
    # total_dic = data_cur.fetchall()
    # data_cur.close()
    # data_conn.close()
    # total_dic_html="""<tr>"""
    # col_html, data_html = ret_html_content(colnames, total_dic, 'add')
    # content = re.sub(r'\{%content%\}', str(total_dic), content)
    # new_content = re.sub(r'\{%table_head%\}', col_html, content)
    # 
    # print(total_dic)
    total_dic_html =""
    for tup_list in total_dic:
        tr_html=f"""<tr>
            <td>{tup_list[0]}</td>
            <td>{tup_list[1]}</td>
            <td>{tup_list[2]}</td>
        </tr>"""
        total_dic_html += tr_html
    new_content = re.sub(r'\{%total_dic%\}', total_dic_html, content)
    # print(search_res)
    # if isinstance(page, str):
    if word:
        # exist matched words
        if search_res!=():
            # print(word, search_res[0])
            
            search_tb =f"""<tr name="search_tr">
                            <td name="search_de">{search_res[0][0]}</td>
                            <td name="search_en">{search_res[0][1]}</td>
                            <td name="search_note">{search_res[0][2]}</td>
                            <td name="search_freq">1</td>
                        </tr>"""
            # new_content = re.sub(r"\{%a%\}", search_tb, new_content)
            # print(search_tb)
            return search_tb
        else:
            return ""
...
def application(environ: dict, set_request_head):
    ...
    try:
        ...
        for url, func in URL_DICT.items():
            ...
            return f'no such file...{func_name=}, and {URL_DICT=}'
    except Exception as e:
        raise
